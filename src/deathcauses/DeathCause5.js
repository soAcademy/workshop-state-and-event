...
const DeathCauseByProvince = (props) => {
  ...
  return (
    <div className="bg-teal-300 w-1/3 p-4">
      <div className="font-bold">Number of Death By Province in {currentYear}</div>
...
const DeathFilter = (props) => {
  ...
  return (
    <div>
      <div>
        เลือกปี พศ {""}
        <select onChange={((e) => setCurrentYear(e.target.value))} value={currentYear}>   {/* value={currentYear} = set default year*/}
          {yearLists.map((year) => (
...
const DeathCause5 = () => {
  const yearLists = [...new Set(ThailandDeathCause.map((r) => r.year))].sort((a ,b) => b - a)
  ...
  const temptDeathByCauses = deathCauseLists.map((cause) => {
  ...
  const temptDeathByProvinces = provinceLists.map((province) => {
