...
const DeathCause5 = () => {
  const yearLists = [...new Set(thailandDeathCause.map((r) => r.year))].sort((a,b)=>a-b);
  const [currentYear, setCurrentYear] = useState(2559);

  const totalDeath = thailandDeathCause.reduce(
    (acc, r) => acc + r.deathMale + r.deathFemale,
    0
  );
  const deathCauseLists = [
    ...new Set(thailandDeathCause.map((cause) => cause.causeOfDeath)),
  ];
  ...
  console.log("deathByCause",deathByCauses);
