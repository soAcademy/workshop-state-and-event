    ...
    <>
      {lotteryResult?.resultTexts?.length > 0 && (  //line 309
        <div className="bg-red-300 m-auto w-1/2  mt-4">
          {lotteryResult?.resultTexts?.map((r, idx) => ( //resultTexts from line 314
            <div className="flex">
...
const LotteryChecker3 = () => {
  const { lotteryData, lotteryDateTitle } = useLotteryData("2022-12-01");
  console.log(lotteryDateTitle);
  const {
    lotteryNumbers,
    setLotteryNumbers,
    toggleCalculateResult,
  ...
  return (
    <div>
      ...
      />
      <LotteryResults lotteryResult={lotteryResult} />
...
export default LotteryChecker3;
