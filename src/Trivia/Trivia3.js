...
const Trivia3 = () => {
  ...
  // ข้างบนเป็น index ของข้อที่เราใช้เพื่อเปลี่ยนไปตามข้อ ข้างล่างเป็นคะแนน
  const [score, setScore] = useState(0);

  const checkAnswer = (quizId, answerId) => {
    //ใช้อันนี้อันเดียวแทนข้างล่างไปเลยก็ได้ เลือกเลยได้ทั้งคู่ setScore(score + (quizes[quizId].answer === answerId ? 1 : 0));
    const newScore = quizes[quizId].answer === answerId ? 1 : 0;
    ...
    <>
      ...
      <div className="grid gap-2 grid-cols-2">
        {quizes[currentQuiz].answers.map((r, index) => (
          <button className="bg-green-400 py-4"
            onClick={() => checkAnswer(currentQuiz, index)}>{r}
          </button>
        ))}
      </div>
    </>
