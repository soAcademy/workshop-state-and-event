    ...
    <div className="w-2/3 mx-auto bg-gray-200 p-4 mt-8 rounded-lg text-left">
      ...
      <div className="grid grid-cols-4 mt-4">
        {provinces.map((province) => (
          <div onClick={() => setProvince(province)} className="cursor-pointer hover:bg-red-200 text-blue-800">{province}</div>
        ))}
      </div>
    </div>
  </>
...
const Zipcode7 = () => {
  ...
  return (
    <div className="w-full text-center">
      {province === undefined && (
        <ZipcodeHome provinces={provinces} setProvince={setProvince} />     //Click = close setProvince = () 
      )}
      {province !== undefined && (
